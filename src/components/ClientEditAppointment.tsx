...
export default function ClientEditAppointment({ appointment, open, onOpenChange }: Props) {
  ...
  const { data: existingAppointments } = useAppointments(dateStr, barberId);
  ...
  const availableSlots = useMemo(() => {
    ...
    if (!blocks) return [];
    const allSlots = generateTimeSlotsFromBlocks(blocks, slotDuration);
    ...
    blockedSlots?.forEach((b) => {
      if (b.barber_id === barberId || !b.barber_id) {
        if (b.all_day) {
    ...
    return allSlots.filter((s) => !bookedTimes.has(s) && !blockedTimes.has(s));
  }, [date, scheduleBlocks, slotDuration, existingAppointments, blockedSlots, barberId, appointment.id]);
